...
def zipper(x, input_x, input_y,  act=lambda x : tl.act.lrelu(x, 0.1), reuse=False, name='zipper', is_train=True,
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        network = tl.layers.ReshapeLayer(network, shape=(-1, observations, input_x, input_y, 1), name='reshape')
        ## 3D-upscale modual
        network = tl.layers.Conv3dLayer(network, shape=[temporal_stride, 3, 3, 1, res3d_map],
        ...
        network = tl.layers.Conv2dLayer(connect11, shape=[4, 4, res2d_map, res2d_map],
                                        strides=[1, 1, 1, 1], padding='SAME', name='cnn2d_layer1',
        ...
        network = tl.layers.Conv2dLayer(network, shape=[4, 4, 3 * res2d_map, 1],
                                        strides=[1, 1, 1, 1], padding='SAME', name='cnn2d_layer4',
                                        act=tl.activation.identity)
