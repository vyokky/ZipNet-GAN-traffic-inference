...
def zipper(x, input_x, input_y, act=lambda x : tl.act.lrelu(x, 0.1), reuse=False, name='zipper', is_train=True,
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        ## Downsample 
        network = tl.layers.PoolLayer(network, ksize=[1,  downscale, downscale, 1], strides=[1, downscale, downscale,1], padding='VALID',
                                      pool=tf.nn.avg_pool, name='pool_layer')
        network = LayerExtension.TransposeLayer(network, (0, 3, 1, 2), name='trans2')
        network = tl.layers.ReshapeLayer(network, shape=(-1, observations, input_x/downscale, input_y/downscale, 1), name='reshape')
        ## 3D-upscale module
	    network = LayerExtension.DeConv3dLayer(layer= network , shape=[1, 5, 5, res3d_map, 1],
                                               output_shape=[-1, observations, input_x/4, input_y/4, res3d_map],
                 strides=[1, 1, 2, 2, 1], padding='VALID',  name='decnn3d_layer0')
        network = tl.layers.BatchNormLayer(network, name = '3d_bn0', act = act,is_train = is_train)
        ...
        network = tl.layers.Conv3dLayer(network, shape=[temporal_stride, 3, 3, res3d_map, res3d_map],
                                          strides=[1, 1, 1, 1, 1], padding='SAME', name='cnn3d_layer7',  act=act)
        network = tl.layers.Conv3dLayer(network, shape=[temporal_stride, 3, 3, res3d_map, res3d_map],
                                          strides=[1, 1, 1, 1, 1], padding='SAME', name='cnn3d_layer8',  act=act)
        network = tl.layers.Conv3dLayer(network, shape=[temporal_stride, 3, 3, res3d_map, res3d_map],
                                          strides=[1, 1, 1, 1, 1], padding='SAME', name='cnn3d_layer9',  act=act)
